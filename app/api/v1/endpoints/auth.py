...
from app.schemas.auth import TokenResponseSchema, LoginRequestSchema, RegisterRequestSchema
...
async def login(
    ...
    service=AuthService(db)
    access_token = await service.login_user(email=payload_forn.username, password=payload_forn.password)
